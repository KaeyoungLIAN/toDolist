...
| 日期 | 版本 | 内容 |
|------|------|------|
| 2026-05-19 | — | **回收站系统**：移除撤销栏，改为回收站模态框（FIFO 最多 10 条），支持逐条恢复 / 清空<br>**数据自动备份**：每次保存原子写入后备份 `.json.bak`，主文件损坏自动读取备份<br>**通知权限检查**：检测 Denied / Prompt 状态，按需请求系统通知权限<br>**每周提醒跳过**：当天已完成则跳过推送，不再重复打扰 |
| 2026-05-18 | — | **提前提醒**：支持设置提前 N 分钟通知<br>**TimePicker**：滚轮方向修正、键盘数字输入 |
| 2026-05-17 | — | **窗口折叠模式**：按 `` ` `` 键折叠为迷你栏，可拖拽 / 置顶 / 显示剩余数，再次按 `` ` `` 恢复<br>**玻璃效果开关**：设置中可关闭/开启毛玻璃（Windows Acrylic）<br>**窗口置顶**：标题栏支持置顶切换 |
| 2026-05-14 | — | **全局搜索**：全屏覆盖层，按内容搜索所有日期，结果按日期分组<br>**链接 / 会议整合**：任务支持链接（网页 / 腾讯会议），腾讯会议自动 `wemeet://` 原生协议一键入会 |
| 2026-05-13 | — | **亮色主题**：CSS 变量体系全量适配，暗色/亮色可切换并持久化<br>**欢迎弹窗**：首次使用引导，可勾选"不再显示"<br>**帮助弹窗**：`?` 按钮打开操作说明，SVG 图标与控件一致<br>**i18n**：全部 UI 字符串中英双语，默认中文<br>**毛玻璃**：窗口 16px 圆角、玻璃可见边框、弹窗透明度 / 对比度优化 |
| 2026-05-12 | — | **taste-ui 改造**：液体玻璃（`backdrop-filter: blur(24px)` + 边缘高光 + 内阴影）、弹簧物理（`cubic-bezier(0.16,1,0.3,1)`）<br>**存档系统**：过去任务自动归档，可恢复<br>**DWM Acrylic**：窗口级毛玻璃 + CSS 重构 |
| 2026-05-11 | v2.0 | **GlassTodo v2.0**：从 Tauri 1 迁移至 Tauri 2.11，Rust + React 19 + Vite 6<br>约 5MB 单文件，零运行时依赖<br>支持单次/每周提醒、日期导航、置顶、排序、暗色主题、中英双语 |
